...
% load sigma coordinate SSC data and convert adcp to sigma coordinates:
...
load('.mat')% load adcp data
load('.mat') %load ssc data
...
for jj=1:length(adcp)
    ...
    % otherwise, put in the nearest matching ssc transect
    idx=find(sscidx==jj);
     if isempty(idx)==1
        diff=abs(jj-sscidx);
        [~,idx]=min(diff);
    ...
    binsize=adcp(jj).interpdepths/(L-5); %sigma bin size
