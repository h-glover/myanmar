...
cd C:\GLOVER\data\myanmar\AQD\BR_Aqd_Mar18\MMICrocStn
aqd=aqdlr2x('MMLG1801.hdr');
...
offset = aqd.atm_pres(end) - aqd.pres(end);
...
idx = [80:90,160:170,225:235,298:308,370:380,447:458,520:530];
%remove bad data at beginning and end, and nan some bad data intervals
for jj=[1:10,21:22]
    % remove data from 1D data fields
    aqd.(fields{jj})(end)=[];
%     if jj>1
%         aqd.(fields{jj})(idx)=NaN;
%     end
% figure; plot(aqd.(fields{jj})),title(fields{jj})
end
% remove bad velocity/accel data when instrument moved
swtch = abs((aqd.roll(3:end)-aqd.roll(1:end-2))./2);
[~,swtch_idx]=sort(swtch,'descend');
swtch_idx = swtch_idx(1:13);
for jj=13:18
    % remove data from 2D data fields
    aqd.(fields{jj})(end,:)=[];
    for kk=1:length(swtch_idx)
        aqd.(fields{jj})(swtch_idx(kk)-4:swtch_idx(kk)+9,:)=NaN;
    end
end
...
% then convert mV to SSC;AGD11782
aqd.ssc1 = aqd.ext1*0.12;
aqd.ssc2 = aqd.ext2*0.12;
