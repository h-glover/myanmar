...
cd C:\GLOVER\data\myanmar\AQD\BR_Aqd_Mar18\MMIsmallchannel
aqd=aqdlr2x('MMsm1801.hdr');
...
offset = nanmean(aqd.atm_pres(1:40)) - nanmean(aqd.pres(1:40));
aqd.depth = aqd.pres+offset-aqd.atm_pres;
aqd.depth(aqd.depth<0.2)=NaN;
...
for jj=[1:10,21:22]
    ...
    aqd.(fields{jj})([1:90,1155:end])=[];
%     if jj>1
%         aqd.(fields{jj})([1:45,1155:end])=NaN;
%     end
...
aqd.ssc2 = NaN(size(aqd.ssc1));
aqd.ssc1=movmean(aqd.ssc1,3);
