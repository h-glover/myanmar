...
names = {'BR_Mar18_Spring';'BR_Mar18_Neap';'YR_Jan19_Neap'};
...
for jj=1:length(adcp)
    ...
    adcp(jj).sal=abs(fillmissing(adcp(jj).sal,'linear',2));
    ...
    adcp(jj).S=adcp(jj).sal.*dA(jj,:);%mg/l to kg/m
...
fluxdecomp.MeasureTime=MeasuredTime(1:end-1);
fluxdecomp.A=A;
fluxdecomp.A0=A0;
fluxdecomp.dA=dA;
fluxdecomp.dA0=dA0;
...
save([names{ff},'_SaltFluxDecomp3'],'fluxdecomp')
