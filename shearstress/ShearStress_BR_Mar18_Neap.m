...
for jj=1:length(adcp)
    ...
    cols=find(isnan(adcp(jj).interpdepths(2:end)));
    for nn=cols(2:end)
        adcp(jj).spd_hab(:,nn)=fillmissing(adcp(jj).spd_hab(:,nn),'previous',2);
    end
    
    MeasuredTime(jj)=adcp(jj).time(1);

%      figure;
%      pcolor(0:1000,hab_mat,adcp(jj).spd_hab),shading flat, colorbar
%      caxis([-100 100])
end
...
%     plot(xx,yfit,'r')
    ...
    if pfit(1,1)<0 || avgs(ii).tau_lotw(jj,kk)>15
...
save('BR_Mar18_Neap_ShearStress','avgs')
