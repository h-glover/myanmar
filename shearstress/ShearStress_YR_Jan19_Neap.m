...
for jj=1:length(adcp)
    if jj<12
        adcp(jj).interpSpeed=[sqrt(adcp(jj).interpalong.^2 +...
        adcp(jj).interpacross.^2);NaN(12,length(adcp(jj).interpdepths))];
    else
    adcp(jj).interpSpeed=sqrt(adcp(jj).interpalong.^2 +...
        adcp(jj).interpacross.^2);
    end
...
for jj=3 % calc Cd for each station
    p=[];xx=[];yy=[];
...
save('YR_Jan19_Neap_ShearStress','avgs')
