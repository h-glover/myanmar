...
% end
% % save: adcp time and index will be reused in the SedFlux calculation
...
for jj=2:length(ctd)
    ...
    if ctd(jj).transect==ctd(jj-1).transect %transect is same as prev cast
        ...
        sal(counter).SSC(1:L,col)=ctd(jj).Salinity;
        ...
        sal(counter).SSC=NaN([rows,3]);
        sal(counter).SSC(1:L,1)=ctd(jj).Salinity;
...
%     figure;plot(sal(counter).interpalong(:,ctd(jj).dist),interpdepth)
%     axis ij
...
%         sal(jj).SSC(:,2:3)=sal(jj-1).SSC(:,2:3);
