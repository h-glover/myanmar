...
for jj=1:length(ctd)
    ...
    % simple QC looking for SSC values that jump by more than 100mg/L
    diff=abs(ctd(jj).SSCCal(1:end-1)-ctd(jj).SSCCal(2:end));
    diff=[diff;0];
    ctd(jj).SSCCal(diff>100)=NaN;
    diff=abs(ctd(jj).SSCCal(1:end-1)-ctd(jj).SSCCal(2:end));
    diff=[diff;0]; diff(isnan(diff))=300;
    ctd(jj).SSCCal(diff>100)=NaN;
end
rows=max(L);
