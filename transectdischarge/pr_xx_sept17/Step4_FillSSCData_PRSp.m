...
for jj=1:length(ctd)
    ...
    if ctd(jj).transect<=2
        nn=1;
    elseif ctd(jj).transect>2 && ctd(jj).transect<=7
        nn=2;
    elseif ctd(jj).transect>7
        nn=3;
    end    
    ...
    ctdproj = proj([left(nn).x - ctd(jj).x, left(nn).y - ctd(jj).y],...
        [left(nn).x- right(nn).x, left(nn).y - right(nn).y]);
...
% 0:0.005:20m deep (change to exceed max depth of adcp data)
interpdepth=0:0.05:40;
...
for jj=2:length(ctd)
    ...
    if ctd(jj).transect==ctd(jj-1).transect %transect is same as prev cast
        ...
        ssc(counter).time=NaN([rows,2]);
        ssc(counter).time(1:L,1)=ctd(jj).time;
        ssc(counter).Depth=NaN([rows,2]);
        ssc(counter).Depth(1:L,1)=ctd(jj).Depth;
        ssc(counter).SSC=NaN([rows,2]);
...
for jj=1:length(ssc)
    if jj==2
        ssc(jj).adcpbeddepth=fillmissing(ssc(jj).adcpbeddepth,'nearest');
    end
    ...
    ssc(jj).sigmaAlongComplete=NaN([length(sigmadepth),2201]);
...
for jj=1:length(ssc)
    ...
    figure;
    pcolor(0:2200,sigmadepth,ssc(jj).sigmaAlongComplete)
    shading flat,axis ij,colorbar
end



save('PR_Sept17_Spring_SSC','ssc')