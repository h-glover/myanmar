...
MeasuredDis(ii)=nansum(nansum(adcp(ii).alongComplete./100)).*0.5;
     adcp(ii).time=datenum(adcp(ii).year+2000,adcp(ii).month,adcp(ii).day,...
         adcp(ii).hour,adcp(ii).minute,zeros(1,length(adcp(ii).minute)));
...
save('YR_discharge_interp','MeasuredDis','InterpDis','InterpTimes','InterpRes')
%%
clear all
...
save('YR_Sept17_discharge_interp','Interp*','Measured*')
%%
